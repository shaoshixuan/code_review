#!/usr/bin/env bash
# =============================================================================
# IERec 流水线 v2 — 跳过已完成的 grocery，运行 automotive + toys
# 用法: nohup caffeinate -i bash run_ierec_pipeline_v2.sh > /tmp/ierec_pipeline_v2.log 2>&1 &
# =============================================================================
set -euo pipefail
...
for DATASET in automotive toys; do
    ...
    if [[ -n "$LATEST_MODEL" ]]; then
        ...
        log "Training IERec on $DATASET (epochs=30, eval_step=10, early_stop=3)..."
        python3 -c "
...
run_recbole(
    ...
    if [[ -f "$OUTPUT_FILE" ]]; then
        warn "Results exist, skip eval: $OUTPUT_FILE"
    else
        LATEST_MODEL=$(ls "$IEREC/saved/"NEW-"$DATASET"*.pth 2>/dev/null | sort | tail -1 || echo "")
        if [[ -z "$LATEST_MODEL" ]]; then
            err "No saved model for $DATASET!"
        fi
        log "Running 100-candidate evaluation for $DATASET..."
...
for DATASET in grocery automotive toys; do
    RESULT="$IEREC/output/$DATASET/ierec_100cand_results.txt"
    if [[ -f "$RESULT" ]]; then
        log "  $DATASET: $RESULT"
        grep -E "Recall|NDCG|CatCov|ILD|Novelty" "$RESULT" | grep "@20" | head -5
    else
        warn "  $DATASET: NO RESULTS"
    fi
done
