...
class Config(object):
    ...
    def compatibility_settings(self):
        ...
        # NumPy 2.0 removed legacy aliases; wrap with try/except
        try: np.bool = np.bool_
        except AttributeError: pass
        try: np.int = np.int_
        except AttributeError: pass
        try: np.float = np.float_
        except AttributeError: np.float = float
        try: np.complex = np.complex_
        except AttributeError: pass
        try: np.object = np.object_
        except AttributeError: np.object = object
        try: np.str = np.str_
        except AttributeError: np.str = str
        try: np.long = np.int_
        except AttributeError: np.long = int
        try: np.unicode = np.unicode_
        except AttributeError: np.unicode = str
